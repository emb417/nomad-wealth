...
class TaxCalculator:
    ...
    def _taxable_social_security(self,
                                 ss_benefits: int,
                                 other_income: int) -> int:
    ...
    def _calculate_ordinary_tax(self, income: int) -> int:
        ...
        for i, (lower, rate) in enumerate(self.brackets):
            upper = self.brackets[i + 1][0] if i + 1 < len(self.brackets) else float("inf")
            if income > lower:
    ...
    def apply_tax(self,
        ...
        ordinary_tax = self._calculate_ordinary_tax(ordinary_income)
        gains_tax    = self._calculate_capital_gains_tax(ordinary_income, gains)
        total_tax    = int(round(ordinary_tax + gains_tax))

        # Withdraw from Cash
        buckets["Cash"].deposit(-total_tax)

        # Refill Cash from other buckets
        refill_txns = self.refill_policy.generate(buckets)
        for tx in refill_txns:
            tx.apply(buckets, pd.Timestamp.now().to_period("M"))  # or pass in forecast_date if available
